...
DataLogger::DataLogger(ros::NodeHandle &node, ros::NodeHandle &priv_nh)
{
    ...
    lasStartcandump = ros::Time::now();
    system("~/Scripts/startcandump");  // call a hard-coded script

}

DataLogger::~DataLogger(){}

/* callback for topic /raptor_state */
void DataLogger::recordRosbagAndCANlog(const deeporange14_msgs::RaptorStateMsg::ConstPtr& msg)
{
  ...
  // toggle loging if the value of log_cmd changes
  if (msg->log_cmd && !isRecording)
  {
    // start both ROS and CAN logging
    // runs in the background and terminates when logging is stopped
    system("~/Scripts/startlogging &");  // call a hard-coded script

    isRecording = true;
    
    ROS_INFO("Started CAN and ROS logging.  Current System State = %d", msg->system_state);
  }
  else if(!msg->log_cmd==0 && isRecording)
  {
    // stop both ROS and CAN logging
    system("~/Scripts/stoplogging");  // call a hard-coded script
