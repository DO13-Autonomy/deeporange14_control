...
{
DeepOrangeStateSupervisor::DeepOrangeStateSupervisor(rclcpp::Node::SharedPtr node)
: node_(node)
{
  // Instantiate sub/pubs
  // TODO - manage QOS for all subscriptions to match the "TCPNoDelay" transport hint
  sub_missionStatus = node->create_subscription<std_msgs::msg::String>(
    std::string(topic_ns + "/mission_status"),
    10,
    std::bind(&DeepOrangeStateSupervisor::getMissionStatus, this, std::placeholders::_1));
  sub_stopRos = node->create_subscription<std_msgs::msg::Bool>(
    std::string(topic_ns + "/stop_ros"),
    10,
  ...
  sub_cmdVel = node->create_subscription<geometry_msgs::msg::Twist>(
  ...
  */
  ...
  timer = node->create_wall_timer(
    std::chrono::duration<double, std::nano>(1.0 / update_freq),
  ...
  switch (state) {
    ...
    case AU_1_STARTUP:
      ...
      if (raptor_hb_detected) {
        RCLCPP_WARN(
          node_->get_logger(),
          "[%s]: RaptorHandshake established, transitioning to AU_2_IDLE",
          state_str.c_str());
        prevSt = 1;
        state = AU_2_IDLE;
        break;
    ...
    case AU_4_DISENGAGING_BRAKES:
      ...
      if (!raptor_hb_detected) {
        ...
        RCLCPP_WARN(
          node_->get_logger(),
          "[%s]: Brakes disengaged, transitioning to ROS controlled, about to move",
          state_str.c_str());
        break;
      } else {
        // do nothing
    ...
    case AU_5_ROS_CONTROLLED:
      ...
      if (!raptor_hb_detected) {
        ...
        RCLCPP_INFO(
          node_->get_logger(),
          "[%s]: Mission Completed or Cancelled, going back to AU_3_ROS_MODE_EN",
          state_str.c_str());
        break;
      } else {
